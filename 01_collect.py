...
def _poll_positions(client: GameClient, stop_evt: threading.Event,
    ...
    """Background thread: poll position via HTTP so the browser WS stays connected."""
    interval = 1.0 / hz
    while not stop_evt.is_set():
        try:
            sensors = client.get_sensors()
            pos = sensors.get("position") if sensors else None
...
def main():
    ...
    session = client.create_session(
        ...
        config={"terrain_seed": args.seed, "obstacles_enabled": True},
    )
    ...
    print("Waiting for browser to fully connect...", flush=True)
    for attempt in range(10):
        ...
        try:
            ...
            body = body.text if body is not None else str(e)
            print(f"  Attempt {attempt+1}/10 failed: {body}", flush=True)
    else:
        raise RuntimeError("Could not start recording after 10 attempts. Is the browser tab open and the game loaded?")
