...
class GameClient:
    ...
    def get_ground_grid(self) -> dict:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/sensors/grid")
        resp.raise_for_status()
        return resp.json()

    def get_grid_observation(self) -> np.ndarray:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/sensors/grid32")
        resp.raise_for_status()
        data = resp.json()
    ...
    def get_targets(self) -> list:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/targets")
        resp.raise_for_status()
        return resp.json()["targets"]

    def get_wind(self) -> dict:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/wind")
        resp.raise_for_status()
        return resp.json()
    ...
    def get_recording(self) -> dict:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/recording")
        resp.raise_for_status()
        return resp.json()
    ...
    def get_recording_as_arrays(self) -> tuple:
        ...
        for sample in recording["samples"]:
            s = sample["state"]
            state_vec = [
                s["speed"], s["heading_error"], s["checkpoint_distance"],
                *s["rays"], s["ground_friction"],
            ]
            actions.append([sample["action"]["throttle"], sample["action"]["steering"]])
    ...
    def get_recording_with_grid(self) -> tuple:
        ...
        return np.array(states, dtype=np.float32), np.array(actions, dtype=np.float32), grid_stack

    # -- Map & Exploration -------------------------------------------------

    def get_explored_map(self) -> dict:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/map/explored")
        resp.raise_for_status()
        return resp.json()

    def get_terrain_ground_truth(self) -> list:
        self._check_session()
        resp = self._http.get(f"{self.server_url}/api/session/{self.session_id}/map/terrain")
        resp.raise_for_status()
...
class RoomBot:
        ...
        try:
            while not self._stop.is_set():
                ...
                if avg_dt > dt_warn_threshold:
                    print(f"[RoomBot:{self.name}] WARNING: controller avg dt {avg_dt*1000:.0f}ms exceeds budget")
                    avg_dt = period
                next_tick += period
    ...
    def _on_message(self, ws, raw):
        ...
        if t == "bot_assigned":
            ...
            with self._lock:
                ...
                self._last_pos_for_speed = None
                self._last_speed = 0.0
            print(f"[RoomBot:{self.name}] round_start idx={ridx} seed={msg.get('seed')} obstacles={msg.get('obstacles')}")
    ...
    def _build_obs(self) -> dict:
        ...
        heading = _quat_to_yaw(
            float(rot.get("x", 0.0)), float(rot.get("y", 0.0)),
